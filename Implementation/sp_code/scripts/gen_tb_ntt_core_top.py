...
def main() -> int:
    ap = argparse.ArgumentParser(description="Generate Verilog TB for ntt_core_top")
    ap.add_argument("--out", type=Path, required=True)
    args = ap.parse_args()

    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(TB_TEXT, encoding="utf-8")
    print(f"TB_OUT={args.out}")
    return 0
